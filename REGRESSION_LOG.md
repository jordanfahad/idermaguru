...
### R-050 — The pause was the echo of the shopper's own answer
- **Symptom:** big pauses between sentences persisted after the blob pipelining, and a listening session on iOS could still sit dead.
- **Cause (pause):** "Got it — sensitive skin. One safety check…" — the question half was cached, but the acknowledgement in front of it contains the shopper's answer, so it read as dynamic and paid a full TTS synthesis on every turn, precisely at the moment the shopper had just spoken. There are only five skin types, one pregnancy line and one no-allergies line: enumerable, now enumerated into `fixedLines` (106 cacheable lines per language).
- **Cause (stuck):** iOS sometimes opens a recognition session whose audio route never engages — no result of ANY kind arrives, and the give-up counter only ticks on `onend`, which a dead session may not fire for a long time. A session that has heard nothing after 7 seconds is now cycled by force, and the existing counter still stands down after repeated silence.
- **Also:** after a photo, the look-line synthesis, the API round trip and the question were three serial waits. `send` is split into `requestTurn`/`handleTurn`, and the photo path runs the round trip UNDER the spoken look — the question follows the look with no third wait.

### R-051 — The orb's light show was starving the microphone
- **Symptom:** third report of the same shape — the orb sat on listening, the shopper said "no" over and over, and nothing happened. Each earlier fix (interim flush R-047, 7-second watchdog R-050) was real and stayed, yet on the shopper's iPhone the session after a TTS reply could still go deaf. Headless tests never reproduced it once.
- **Cause:** `listen()` called `orbAudio.attachMic()` on every session, which opens `getUserMedia({ audio: true })` and HOLDS the stream — its only purpose is amplitude for the orb animation. iOS Safari does not multiplex the audio-input route the way desktop Chrome does: with a page-held capture stream open, `webkitSpeechRecognition` can be handed no audio at all — no interim, no final, no error, ever. The TTS playback in between reshuffles the audio session, which is why it worked early in the conversation and died right after the advisor spoke. Desktop Chrome shares the route happily, which is exactly why every headless test passed while every iPhone got stuck.
- **Fix:** on iOS the orb gives up its amplitude stream while recognition runs (`detachMic()` instead of `attachMic()`); the CSS listening ripple still signals the state. Everywhere else the animation stays. And the give-up paths no longer stand down in silence — standing down without a word is indistinguishable from being stuck, so both the watchdog and the onend counter now say: "I'm not hearing anything from the microphone. Tap the orb to try again — or switch to Chat and type, that always works."
- **Falsifiable check if it ever sticks again:** did grey interim words appear before it stuck? Words-then-stuck and silence-then-stuck now have different remaining suspects (send path vs audio route).

### R-052 — "I don't like oil" was asked which one, with face-step examples
- **The good news first:** the same screenshot proved R-051 fixed — "No" and "No not at all" both landed by voice on the shopper's iPhone, at the exact question where the mic died three times before.
- **Symptom:** straight after a dandruff routine of shampoo, conditioner and a scalp oil, "I don't like oil" got: "Happy to change it — which one? Tell me the product or the step, like 'the cleanser' or 'the serum'." The shopper NAMED the only oil on their screen and was asked to name it — with example steps from a face routine they weren't looking at.
- **Cause:** the debate flow always rebuilt the routine with the face builder, whatever builder had actually produced it. A hair shopper's challenge was matched against a face routine that contains no oil, so nothing matched; and `whichSwap` was one fixed sentence with face examples baked in. STEP_WORDS had no hair vocabulary either — shampoo, conditioner, scalp and oil pointed at nothing.
- **Fix:** the followup handler now rebuilds with the same builder the routine came from (hair picker, body picker, or face builder), all normalised to one shape, so why/swap/which-one work identically on all three. STEP_WORDS learned the hair steps in both languages. `whichSwap` takes the on-screen step labels and quotes the first two — "like 'the shampoo' or 'the conditioner'" — with the common variants pre-cached per language. And the hair and body result paths now honour `dislikedIds`, so a swap survives a later "make it stronger" instead of quietly bringing the disliked product back (the face path already did, via excludeProductIds).
- **Verified** by six route-level tests over a mocked hair catalogue: the oil swap happens and is named, the swap survives a rebuild, "why the shampoo?" gets the reasoning, the which-one examples name hair steps and never face steps, and a dislike with no replacement left is answered honestly.

### R-053 — The acne after the dandruff routine was never heard
- **Symptom:** a shopper with a fresh three-product dandruff routine said they wanted to add something for their acne — and the advisor "kept asking me to buy the three oils": the same hair routine, re-read at them, while the acne went unanswered.
- **Cause:** "I want to add more for my acne" contains "add more", which the adjustment reader owns — so the turn became "make the dandruff routine fuller", the hair picker rebuilt the same three products (nothing more to add), and the same-again line replayed. No reader existed for a NEW concern; the interview had no way to change topic, and nothing ever asked whether the shopper wanted anything else, so "no, I'm done" had nowhere to go but the tangent classifier.
- **Fix, three parts:**
  - `readNewConcern`: a named concern absent from the current one (acne over dandruff) outranks every routine follow-up and starts a fresh interview via `beginConcern` — which keeps the PERSON (skin type, pregnancy, allergies, age, dislikes all carry; nothing is re-asked) and resets only what was scoped to the old concern. The switch is welcomed out loud: "Happy to help with that too — let's sort it."
  - Every fresh routine now ends with the door held open, as its own pre-cached spoken part: "Anything else I can help with — your skin, your hair? Or if that's everything, I'll leave you to look through it." A bare "yes" gets "Go on — what else is bothering you?" (and the next utterance IS the concern, whatever words it uses); "no thanks" gets a warm wrap-up on a new `farewell` phase, and the client leaves the microphone closed after goodbye.
  - Both readers are whole-utterance and consulted only once the routine is settled, so a mid-interview "no" still belongs to the open question.
- **Verified** by nine route-level tests (the switch, no pregnancy re-ask, the closer, the farewell, the bare-yes flow, person-carries-over) and a real-browser run of the whole conversation: routine → closer → "no thanks" → farewell, with the microphone provably staying closed afterwards.

### R-054 — The voice started strong and fell to a whisper
- **Symptom:** "the voice gets high at the beginning and extremely low" — the first lines played at full volume and later lines went very quiet, on iPhone.
- **Cause, two iOS behaviours stacked:** (1) `speak()` captured the audio element into the orb's WebAudio graph (`createMediaElementSource`) so the orb could pulse with the agent's voice. That capture is permanent for the element — and iOS pins the graph's output at the ducked level it had during the last speech-recognition session, so every line after the first listen played at recognition-era volume. The greeting, played before any recognition had ever run, was the one loud line. (2) A silent-restart timer queued just before a reply arrived could reopen recognition DURING playback, and iOS ducks media output to a whisper while capture is live.
- **Fix:** on iOS the reply element is never captured into the graph — it plays on the normal media path, which iOS restores to full volume (the orb keeps its CSS speaking state, consistent with R-051's choice on the listening side). `speak()` now also clears any queued mic restart and aborts a live recognition session before a word is played — the advisor never talks over its own open microphone — and pins `volume = 1`.

### R-055 — "Hello can you hear me" got the brush-off, and the mic could die without a word
- **Symptom (screenshot):** the very first exchange of a session was "Hello can you hear me" → "That one's outside my world, I'm afraid" — and then the conversation stopped: production logs show the prewarm healthy, exactly one turn request, and silence after it.
- **Cause 1 — the brush-off:** "hello can you hear me" is five words, one over the greeting classifier's limit, and mentions nothing in the skin vocabulary — so a person checking the microphone works was told their words were outside the advisor's world. To someone testing the thing, that reads as a machine failing a human test.
- **Cause 2 — the silent stop:** `recognition.start()` throws on iOS when a session is started during an audio-route transition — precisely the moment playback ends and listening should resume. The catch set the phase to idle: no retry, no message, conversation over. Any turn could end the session this way.
- **Cause 3 — the wrong stand-down message:** after ~30 quiet seconds the advisor stood down saying "I'm not hearing anything from the microphone" — even when the microphone had been working all along and the shopper was just thinking. A working mic gone quiet is a person, not a fault.
- **Fix:** a hearing check is read in both languages and answered like a person — "Loud and clear — I can hear you!" — at the opening and mid-conversation (where the open question is then repeated). A `start()` throw now retries with the same patience budget as every other silent failure instead of dying quietly. And the stand-down message depends on history: a mic that has produced words this visit gets "Still with you — tap the orb whenever you're ready to carry on."; only a mic that has never produced anything gets the trouble message.
- **Verified** by three route tests (opening check, mid-conversation check, non-firing on "hello"/"my ears are dry") and a real-browser run in which `start()` throws twice mid-conversation and the retry still carries the session to a full routine.

### R-056 — "Do you have any hair serum?" was answered by re-reading the routine, three times
- **Symptom (transcript):** after a dandruff routine, the shopper asked "What about hair serum do you have any hair serum", then "what about multiply hair serum" [speech-to-text for Multi-Peptide], then "I'm asking about the ordinary multi peptide hair salon do you have it" [STT for hair serum]. All three times: "Here's a 3-step hair and scalp routine from the store…" — the identical routine, re-read verbatim. The store stocks The Ordinary Multi-Peptide Serum for Hair Density.
- **Cause 1:** no reader existed for a question about a specific product. The words fell through every follow-up reader into the result flow, which rebuilt the same routine — and unlike the face path, the hair path had no same-as-before guard, so it read the whole line out again rather than saying "same steps — tell me what you'd change".
- **Cause 2 (the earlier swap refusal):** the debate flow's "that's genuinely the only product that fits that step" was said when alternatives exist in the catalogue but were dropped by the live stock check — untrue as phrased.
- **Fix:**
  - `readProductQuery` + `findProductByQuery`: a product question is recognised in its natural shapes and matched against the catalogue by token scoring with prefix credit, so "multiply hair serum" finds "Multi-Peptide Serum for Hair Density" through the transcription noise. Found and buyable → swapped into its step ("Good shout — the store has X. In it goes, out comes Y") and PINNED, so a later "make it stronger" keeps the product they asked for by name; pins still pass the hard safety filters, and a disliked product loses its pin. Found but sold out → says exactly that. Not stocked → says exactly that, with no transcript echo. Already in the routine → defends it with its reasoning.
  - Hair and body results now carry the same same-as-before guard the face path has: an identical rebuild says "I'd still put you on the same N steps — tell me what you'd change", never the full re-read.
  - The swap refusal distinguishes "nothing else fits" from "the alternative is out of stock right now" — a new honest line for the second case.
- **Verified** by eight route tests replaying the shopper's exact three phrasings against the merchant's real catalogue rows: the serum is found through STT noise, swapped in, pinned across rebuilds; sold-out and not-stocked both answered honestly; and the hair routine never re-reads itself.

### R-057 — "I have hair dandruff" was read as a pregnancy, and an opening product question as a concern
- **Symptom 1 (transcript):** at the pregnancy question, the shopper repeated "Hey I have hair dandruff and acne" — and was answered "Got it — I'll skip the ingredients that aren't advised." The advisor recorded a pregnancy off a sentence about dandruff.
- **Cause:** the yes/no reader counts "i have" as a yes-marker (for answers like "yes I have"), and the pregnancy reader falls back to yes/no when nothing explicit is said. "Hey I HAVE hair dandruff and acne" therefore read as "yes". The same trap sat in the allergy question.
- **Fix:** a sentence that is busy describing a skin or hair concern (`describesConcern`) is never a yes to a safety question. The restated concern is APPENDED to the main concern — it was new information, the acne now has a home — and the open question is asked again. Applied to both the pregnancy and allergy readers.
- **Symptom 2:** "Do you have a Miley hair oil" [STT for Mielle] as the very first words was swallowed into the interview as a concern — the question was never answered.
- **Fix:** product questions now work at any point in the conversation. Before a routine exists: the product is confirmed ("Good news — the store has X. I've put it on screen and I'll keep it in whatever we build"), shown as a card, PINNED into whatever routine is eventually built, and the interview carries on with the open question. Sold-out and blocked cases answer honestly and continue. The matcher gained edit-distance credit so "Miley" finds "Mielle". Guarded off the allergen-list and body-area answers, where "what about my hands" is an answer, not a shopping query.
- **Symptom 3:** an opening naming TWO concerns ("hair dandruff and acne on my face") got a bare "Got it" — the shopper repeated themselves because the acne sounded ignored.
- **Fix:** a two-concern opening is told the plan out loud: "Got it — that's two things, so let's take them one at a time. Hair first." The closer at the end of the routine already brings the conversation back for the second.

### R-058 — The voice went quiet in the middle, with long pauses either side
- **Symptom:** "the voice becomes very low in the middle and strong long pauses" — the cached lines played instantly in the natural voice, and precisely the lines carrying the shopper's own details (the allergy read-back, the routine summary, product names) arrived late, or in the quiet robotic browser voice.
- **Cause 1 — dynamic lines were synthesised in their turn, not ahead of it.** `speakSequence` pre-fetched only the FIXED lines as blobs; a dynamic part waited until the parts before it had finished playing and then paid its entire synthesis (1–3s) as an audible pause in the middle of the reply. A failure at that moment dropped that one line — and only that line — to the browser voice: quieter, robotic, mid-sentence-of-the-conversation.
- **Cause 2 — the biggest reply was welded into one dynamic part.** The result turn joined acknowledgement + result line + closer into a single string; the presence of five dynamic words at the front made the whole thing uncacheable, every time, on the most important turn in the product.
- **Fix:** every part of every reply is now synthesised in parallel the moment the reply arrives — fixed lines over GET, dynamic ones over POST — so later parts are ready in memory while the first is still playing. Result replies are split into their natural parts (acknowledgement | result line | photo note | closer), so everything but the short acknowledgement plays from cache. The TTS direction now explicitly asks for full, consistent projection with no trailing off — and because the recordings are cached immutable by URL, the speech URLs carry a voice-direction version (`v=2`) so the new read actually reaches shoppers instead of the old recordings playing forever. The browser fallback voice is pinned to full volume for the rare case it still speaks.

### R-059 — The voice still went down: the orb's own audio engine was holding the duck
- **Symptom:** third report of the same thing — "voice goes down, it's not fixed" — surviving the element-capture fix (R-054) and the parallel-synthesis fix (R-058).
- **The two causes left standing, both eliminated on iOS:**
  1. The orb's audio engine (ambient hum, turn-taking chime, level analyser) keeps a Web Audio output RENDERING for the entire session. With one live, iOS holds the page's media output at the ducked level it applies while speech recognition runs — so the greeting (played before any recognition) was loud, and everything after the first listen played quiet. On iOS the engine is now never created at all: the orb is CSS-animated only, and the reply element owns the audio route outright.
  2. Playback started the instant a reply was ready — often inside the beat iOS takes to lift the duck AFTER recognition ends, and a line that starts ducked stays ducked to its end. Speech now waits out the remainder of a 450ms settle window after the last microphone activity; later parts of the same reply see an old timestamp and start with no delay.
- **Traded away on iOS:** the ambient hum and the audible turn-chime (the visual ripple still marks listening). A fair price for a voice that stays at full volume.
- **Verified** in a real browser: with an iPhone identity the page creates ZERO AudioContexts through a full conversation to the farewell; desktop still creates its one orb engine. If volume still drops after this, the remaining suspect is Apple's recognition service itself, and the fix is server-side transcription over a persistent stream — a build, not a patch.

### R-060 — iOS leaves Apple's speech recognition behind: the call model
- **Symptom:** fifth report of the conversation getting stuck after a question on the shopper's iPhone, surviving every browser-level patch (interim flush R-047, watchdog R-050, mic-hold R-051, start-retry R-055, session hygiene R-059). The component that kept failing was Apple's `webkitSpeechRecognition` itself — sessions that never deliver a result, in ways a page can neither observe nor fix.
- **Fix — stop using it.** On iOS the microphone is now held open for the visit like a phone call. Each turn is RECORDED (MediaRecorder), turn-taking is decided by the audio level (stop a beat after the shopper goes quiet; stand down with the usual patience if nothing was said), and the audio goes to `/api/voice-agent/transcribe` — server-side speech-to-text on the same key the voice already uses. The audio is transcribed and discarded. On-device recognition simply does not run on iOS any more; desktop and Android keep the Web Speech path that has always worked there.
- **This also settles the volume for good:** with the call held open, the audio session stays in ONE state from greeting to goodbye — no duck, no lift, no per-line lottery. The farewell hangs the call up so the mic indicator goes out.
- **And the shopper can always see the conversation (requested):** a fixed call bar at the bottom of the screen shows who has the floor — "You're speaking — I'm listening →" with the live words, "Your advisor speaking →" with the sentence being said — wherever they have scrolled, including down at the routine. ("Dermatologist speaking" was requested; the advisor never claims to be a doctor anywhere in the product, so the bar says advisor for the same compliance reason the intake does.) The existing auto-scroll to a fresh routine stands.
- **Verified** in a real browser: an iPhone-identity session with a pulsing fake microphone ran the entire conversation — five recorded turns, five server transcriptions, routine, closer, farewell — with SpeechRecognition instrumented to throw if touched (it never was), and both call-bar labels observed live. Four route tests cover the transcribe endpoint's failure modes.

### R-061 — The call's turn-meter was deaf, and it had veto power
- **Symptom (screenshot):** on the very first turn of the new call mode — orb LISTENING, call bar showing "You're speaking — I'm listening →", microphone held (Safari's mic icon lit) — the shopper talked and nothing ever happened.
- **Cause, twofold:** (1) the level meter's AudioContext was created when the first turn started — AFTER the tap — and iOS starts a context created outside a user gesture SUSPENDED, where its analyser reads flat silence. (2) The meter had veto power: a turn whose meter saw no voice was discarded as silence instead of transcribed, so a deaf meter meant a deaf advisor, forever, while everything looked alive.
- **Fix:** the analyser's context is created and resumed INSIDE the tap, alongside the call stream. And the meter is demoted from gatekeeper to accelerator: every recorded turn is sent for transcription when it ends — the meter's only job is ending a turn quickly after the shopper goes quiet, and the timers (5s with no detected voice, 15s cap) end every turn regardless. The transcriber is the judge of whether anything was said; an empty transcription feeds the ordinary patience machinery. A dead meter now costs a slower cadence, never a lost word.
- **Verified** in a real browser, in both worlds: a working meter (pulsing microphone, quick turns) AND a completely dead one (AudioContext removed outright — the shopper's iPhone) each carried a full five-turn conversation to routine, closer and farewell, with on-device recognition instrumented to throw if touched.

### R-062 — "I don't like laroche, only Korean brands" swapped one La Roche for another
- **Symptom (transcript):** a brand-level rejection was handled as a single-product swap — and the replacement was ANOTHER product of the same brand. The shopper said "Its still there" and got "That one's outside my world, I'm afraid"; their exasperated follow-up got the let-it-go line. The routine still showed two La Roche-Posay products.
- **Cause 1:** no concept of a brand existed. The swap matcher scored product names, matched the one cleanser, and replaced it with the next-best cleanser — same brand, correctly ranked, completely deaf to the actual request. (Merchant feeds put the STORE's name in the brand column, so the brand lives in the product name.)
- **Cause 2:** "it's still there" names nothing in the skin vocabulary → tangent classifier → "outside my world", twice, at the exact moment trust needed repairing.
- **Fix:**
  - `dislikedBrands`: a rejection whose word matches SEVERAL items on screen (or one, in a brand-shaped sentence — "only …", "… brands") evicts everything whose NAME matches, fuzzily, so speech-to-text's "laroche" finds "La Roche-Posay". The exclusion persists through every later rebuild — including topic switches, because a brand dislike is about the person — and lives in the product LIST, not just the exclusion set, so the sold-out rebuild (which overwrites excludeProductIds) can never resurrect it.
  - `readsStillThere`: the complaint is believed. The sentence is re-read for a brand or product word, every exclusion is applied again, and the answer is either the apology and the fix ("You're right — my mistake, and it's gone now") with the corrected routine, or proof of a clean screen ("I've just double-checked — the one you didn't want isn't in it").
  - "I don't like the cleanser" stays a single-product swap: one name hit and no brand-shaped phrasing.
- **Verified** by seven tests replaying the exact transcript against a two-brand catalogue: the eviction, its persistence through "make it stronger", the apology path from the shopper's actual "Wtf?" sentence, the clean-screen proof, and the single-swap non-regression.

### R-063 — The swap argued about an invisible product, and the catalogue learnt its brands
- **Reproduced from the live transcript, against the merchant's real rows:** "I don't like oil. Replace oil with something else" was answered "that's genuinely the only product in this store that fits that step" — twice — while six products could hold the scalp step. The matcher had scored "oil" highest against the Kate Blanc CASTOR oil (name hit + step-word hit = 5) — a product the LIVE stock check had hidden from the shopper's screen, because the debate flow never applied that filter. It then found no second castor oil and refused, about a product the shopper had never seen. "Remove myli hair oil" wasn't read at all ("remove" was not a swap verb), and "on the website I can still see Ordinary Multipeptide Hair Serum" fell through to same-again.
- **Fixes:** the debate's `current` is stock-filtered — it argues only about the screen. A dislike word matching SEVERAL on-screen items ("oil" → the Mielle oil AND the castor oil) evicts them all, brandDropped-style, instead of single-swapping the wrong one. "Remove / take out / get rid of / don't need" are swap verbs. "Still see/sells/stocks X" joins the grievance reader, and a grievance naming a product that ISN'T on screen is treated as the product request it is: found in the catalogue, checked for stock and safety, pinned and swapped in.
- **And the merchant's classified catalogue is now wired in** (424 products, 73 brands): a generated brand registry keyed by product handle carries the REAL brand and its origin. Product cards stop saying the store's name is the brand, and "only Korean brands please" becomes a standing `preferredOrigin` — honoured in every rebuild, alone or said in the same breath as a dislike, with the honest no-products line when a category has no Korean stock.
- **Verified** by a faithful replay of the transcript against the real hair rows (all three turns now land) and origin tests over a two-origin catalogue — 385 tests passing.

### R-064 — Two concerns kept replacing each other's routines
- **Symptom (transcript):** dandruff routine, then "I have a little bit acne as well" — and the acne routine REPLACED the hair routine on screen. "What about my dandruff? Did you remove it?" brought the hair back and hid the acne; "Add to Cart has only one option, I mean both" got the off-topic brush-off. The advisor could only ever hold one routine.
...
### R-066 — "Advisor speaking" with no sound: capture now fully releases for every reply
- **Symptom (report #6 on voice):** "Advisor speaking but I cannot hear anything. The user experience is not very good." The call bar showed the reply being said — turn machinery, transcription and text all fine — while the phone stayed silent. R-065's track-mute was not enough.
- **Cause:** iOS keeps the page's audio session in CALL mode while ANY capture artefact is alive, and in that state media playback is ducked to nothing or routed to the earpiece. Call mode kept two such artefacts alive through every reply: the microphone track (live, merely muted — muting does not release the session) and the VAD level-meter's AudioContext (running, fed by a microphone source — the exact mechanism R-059 proved silences playback, reintroduced for the turn meter by R-061). Each turn also stacked ANOTHER mic tap onto that context without ever disconnecting the old one.
- **Fix:** the capture stack comes down COMPLETELY for the length of every advisor line — recorder cancelled, microphone tracks STOPPED (not muted), analyser context SUSPENDED, and each turn's meter graph disconnected when the turn ends. The next recorded turn re-acquires the microphone (same page, already granted — no second permission prompt) and resumes the context; if the resume fails the meter is a dead accelerator and the timers still end every turn (R-061's contract). Stopping real capture starts the existing 450ms settle clock so playback never begins inside the session hand-back.
- **And a recovery the shopper can hold:** a "Hear that again" chip sits in the call bar whenever the advisor is listening. One tap replays the whole last reply inside a fresh user gesture — the strongest playback permission iOS grants — so a missed or swallowed line costs a tap, not the conversation.
- **Verified** in a real browser, both worlds: with a working meter the full five-turn call completes with the microphone re-acquired for every turn, ZERO tracks left live, the context suspended for every reply, and the replay chip observed flipping the bar back to "Advisor speaking" mid-call; with no AudioContext at all the call still completes and still releases every track. 386 tests passing.

### R-067 — The mute advisor, found in the logs: her longest lines were being refused by her own server
- **Symptom (report #7, immediately after R-066):** "Not fixed still the same. After I replied the reply from the advisor went mute." Greeting audible, first reply silent, conversation dead — with "Advisor speaking" on the bar.
- **Found, not guessed:** production runtime logs for the exact session show the whole story in four lines — greeting speech GET **200**, the shopper's turn transcribed **200**, the reply computed **200**, then the reply's speech GET **400, twice**. The advisor's reply was REFUSED by our own speech endpoint.
- **Cause:** the speech GET handler capped text at 400 characters, but `fixedLines` — the list the client uses to decide a line is GET-able — contains lines longer than that: the intimate-area answer (508) and the clinician referral (535). Exactly the lines said to a shopper who is worried. A refused GET dropped to the browser voice, and iOS refuses an utterance queued outside a tap SILENTLY — no start, no end, no error — so the advisor showed "speaking" over dead air and the conversation never moved again: mute AND wedged, which is what every "the voice stopped" report actually described.
- **Fixes, layered so no single door can mute her again:**
  - The GET gate is now MEMBERSHIP in `fixedLines` (same source of truth the client reads), not length. The longest lines in the product synthesise and CDN-cache like any other; junk text is refused outright, which also keeps arbitrary text out of the forever-cache.
  - The client never trades the natural voice for a closed cache door: a scripted line whose GET fails retries over POST (in `speak()` and in the sequence preloads) before any fallback.
  - The browser-voice fallback got a watchdog: if speech hasn't audibly started in 3s (iOS's silent refusal) it gives up and moves the conversation on — the words are on screen and "Hear that again" replays them naturally — plus a hard ceiling for lost end-events. The fallback can no longer wedge a call.
  - Microphone acquisition is single-flight: begin()'s in-tap request and the first listen could race into TWO streams, and the orphan — unstoppable, unreferenced — held the iPhone's audio session in call mode for the whole visit. One pending request shared by all callers; hang-up also chases a request still in flight.
  - New `/api/voice-agent/client-log` postbox: the client reports which speech path fired (played / refused / fell back / went mute) as event names and sizes only — never words — so the NEXT on-device playback failure is a log line, not another week of guessing.
- **Verified:** every fixed line in both languages now passes the GET gate (test iterates all 244; the two long lines asserted specifically; junk asserted refused); the in-browser call proof passes in both worlds with zero tracks left live after hang-up (the previous "leak" was the proof sampling before the farewell finished releasing — instrumented timeline shows acquire/stop alternating perfectly). 392 tests passing.

### R-068 — The clinician referral now exists in Arabic (requested)
- **Gap, surfaced by R-067's audit of the long lines:** `ESCALATION_MESSAGE` — the "please see a real clinician" referral, the most sensitive line in the product — existed only in English. `localise()` returns authored languages untouched, so an Arabic conversation that tripped a red flag heard the referral in English, at exactly the moment tone matters most. The shopper asked for both languages.
- **Fix:** the referral is AUTHORED in Arabic (`ESCALATION_MESSAGE_AR`, matching the register of the existing Arabic copy — warm, straight, same urgent-care close), with `escalationMessage(lang)` choosing by conversation language. All three places the voice route can refer — per-turn triage, the model's needs-clinician reading, and the final profile triage — speak it through that helper; triage itself still authors English and the route translates by identity, so anything a merchant might configure later passes through untouched. The Arabic line joins `fixedLines("ar")`: prewarmed, CDN-cacheable, and inside the speech GET gate from birth.
- **Verified** through the real route: a bilingual red-flag utterance in Arabic script gets `phase: referral` with the ARABIC line verbatim; an English session keeps the English one. The fixed-line audits (cacheable, GET-accepted, languages-apart) all cover the new line. 394 tests passing.

### R-069 — The advisor could not say goodbye
- **Symptom (two screenshots of one live session):** the shopper tried to leave three times. "Goodbye." was answered with a rebuilt routine ("I've had another look and I'd still put you on the same 3 steps"), "Goodbye, thank you so much." got "Any time." and the floor held open, and "Bye-bye." got "That one's outside my world, I'm afraid" — the off-topic brush-off, said to a person walking out the door.
- **Cause:** no reader knew a goodbye word. "Goodbye." fell through the done reader (built for "no thanks"-shaped answers) into the follow-up path, which rebuilt and announced same-again; "goodbye + thanks" was claimed by the thanks-aside; "Bye-bye." named nothing in the skin vocabulary and landed in the tangent classifier.
- **Fix:** `readsFarewell` — one or more goodbye words (goodbye, bye, bye-bye, see you, take care, مع السلامة, باي, وداعاً, …) surrounded by nothing but courtesy (thanks, love, okay, شكراً…), whole-utterance, both languages. Any substantive word ("bye bye blackheads") disqualifies it. It is OR-ed into the done guard ahead of every conversational reader, so a farewell ends the visit warmly (wrap-up line, farewell phase, call hung up) from ANYWHERE — over a routine, mid-interview, while the closer waits. Plain thanks without a goodbye still gets "Any time." and an open floor.
- **Verified** by replaying the screenshots through the real route: all three goodbyes now end with the wrap-up and the farewell phase, the thanks-only line stays a warm aside, a mid-interview goodbye leaves too, and مع السلامة closes an Arabic visit. 402 tests passing.

### R-070 — Four holes closed before pointing a customer's domain at us
- **Context:** the go-live audit (custom subdomain + widget embed on the merchant's storefront) found 32 verified blockers. These four had to close before DNS could point anywhere, because three of them were already live on idermaguru.com and the fourth would have shipped the vendor's whole API plane on the merchant's brand.
- **`/dashboard` was public, and served every tenant at once.** No authentication of any kind, reading through the Supabase SERVICE-ROLE key (which bypasses row-level security) with no tenant filter on either query — and listed in `sitemap.xml` for crawlers. Now: `getAdminSession()` is the first statement in the component with `redirect()` before any data access and outside any try/catch (a caught NEXT_REDIRECT would have rendered the console anyway); both reads moved onto the tenant-scoped `withTenant`/Prisma path, because the legacy `outbound_clicks` and `recommendations` tables have no tenant column to filter on; the tenant selector in the URL is honoured only for `super_admin`; `force-dynamic` so it can never be prerendered to a CDN; removed from the sitemap and added to the robots disallow.
- **The advisor host published the entire API.** `ADVISOR_ALLOWED` carried the single entry `"/api/"`, so pointing a merchant subdomain here would have answered `POST /api/admin/auth/login` — unlimited guessing against one hardcoded address — plus `/api/billing/*` and `/api/shopify/install`, on the merchant's own brand. Replaced with the prefixes the advisor actually calls, derived by grepping every `fetch("/api/…")` in the client and re-derived independently in review: `/api/voice-agent`, `/api/cart`, and the four widget endpoints. Matching is now on whole path segments, so `/api/chat` can never lend its allowance to a future `/api/chat-admin`. A blocked API path returns **404, not the 307** a blocked page gets — a 307 preserves method and body, so a POST of admin credentials would have been replayed at `/` by any client that follows redirects.
- **`/auth/callback` was an open redirector.** `new URL(next, origin)` resolves an absolute `next` against the ATTACKER's origin, so a mailed `?next=https://evil.example` bounced visitors off-site wearing our domain. Now only a path starting with a single `/` is accepted; tab/newline/CR are stripped first (the URL parser strips them too, so `"/\t/evil.example"` arrives as `"//evil.example"`) and `/\` is rejected because http(s) parsers read a backslash as a slash.
- **`/api/billing/portal` opened a Stripe portal for whatever customer the body named**, with no session — and a `cus_…` id is printed on every receipt and invoice PDF, so anyone holding one could read a merchant's card details and cancel their plan. Both billing routes are now super-admin gated. The id still comes from the body because nothing maps a merchant to their Stripe customer yet; that is recorded in the route and in the security audit.
- **And the paid endpoints have budgets.** `src/lib/rate-limit.ts` — a dependency-free per-instance limiter on the three unauthenticated routes that proxy a paid model (speech, transcribe, vision), returning 429 with `Retry-After` and `no-store` (a refusal cached against an immutable scripted-line URL would mute that line at that point of presence for a year). The file is honest in its own comment about being per-instance rather than distributed, and about the deployment precondition that decides whether it helps at all: the platform rewrites the forwarded-for header to the immediate peer, so an advisor hostname pointed at us THROUGH another CDN collapses every shopper in the store into one counter — a merchant subdomain must be a direct, unproxied CNAME.
- **Two numbers came out of review, not implementation, and both mattered.** The scripted-speech ceiling is now far clear of the paid one and pinned by a test: a scripted line refused at the free GET door is *re-sent over POST* by the client (R-067 added that fallback so a closed cache door could not mute the advisor), so making the cheap door the first to close would have spent money rather than saved it. And the camera budget went from 30 to 120, because it was sized for one person while every budget here is keyed per ADDRESS — and a refused photo is the only refusal that dead-ends instead of degrading.
- **Verified:** 448 tests passing (402 before), including six that fail against the pre-fix sources; the allowlist re-derived independently against the real client; production build clean.
- **Known follow-up (not a regression, a pre-existing gap now visible):** gating `/dashboard` means the Supabase magic-link flow lands merchants on a page their cookie cannot open. The page never honoured that session — it was simply ungated — so this is the login integration surfacing, not breaking. Owner access via `/admin/login` is unaffected.
